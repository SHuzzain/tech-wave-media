...
describe("WordPress API", () => {
  ...
  describe("getPostsPaginated", () => {
    ...
    it("returns seed posts on fetch failure", async () => {
      ...
      expect(result.headers.total).toBeGreaterThan(0);
    });
  });
  ...
  describe("getAllPostsForSitemap", () => {
    ...
    it("returns seed posts on failure", async () => {
  ...
  describe("getAllCategories", () => {
    ...
    it("returns seed categories on failure", async () => {
      mockFetch.mockRejectedValueOnce(new Error("fail"));

      const { getAllCategories } = await import("@/lib/wordpress");
      const result = await getAllCategories();

      expect(result.map((c) => c.slug)).toContain("business");
    });
  });
